...
mod data_structs;
use data_structs::host_facts::{init_host_facts_struct, HostFacts};
...
async fn main() {
    ...
    println!(
        "Using config options: interval: {}, rhost: {}",
        config_struct.interval, remote_host
    );
    ...
    match config_struct.disks {
        Some(ref conf_disks) => {
            println!(
                "The following disks are configured to be monitored: {:?}",
                conf_disks
            );
            disks_configured = true;
        }
        None => {
            println!("No disks configured in config, disk monitoring disabled");
        }
    };
    ...
    println!(
        "Getting system info with {} second intervals...",
        config_struct.interval
    );
    ...
    println!(
        "The following disks have been detected in the system: {:?}",
        host_facts.disks
    );
    println!(
        "The following hostname has been detected: {}",
        String::from(&host_facts.system.hostname)
    );
    ...
    loop {
        ...
        if rhost_configured {
            send_live_metrics_to_remote(&remote_host, &live_metrics, &host_facts.system.hostname)
                .await;
        }
...
fn print_to_console(live_metrics_struct: &LiveMetrics, host_facts_struct: &HostFacts) {
    ...
    let cpu_freq: f32 = if live_metrics_struct.cpu.freq > 1024. {
        cpu_freq_ext = String::from("GHz");
        live_metrics_struct.cpu.freq / 1024.
    } else {
        live_metrics_struct.cpu.freq
    };
    let cpu_util: f32 =
        live_metrics_struct.cpu.cpu_load.load_1m / host_facts_struct.cpu.cores as f32;

    print!(
        "CPU: cores: {}, freq avg {:.2} {}, load {} {} {}, util {:.1}%; ",
        host_facts_struct.cpu.cores,
        cpu_freq,
        cpu_freq_ext,
        live_metrics_struct.cpu.cpu_load.load_1m,
        live_metrics_struct.cpu.cpu_load.load_5m,
    ...
    if mem_avail > 10u64.pow(9) {
        if mem_cached > 1024 {
            print!(
                "MEM: used {:.2} Gb, free {:.2} Gb, cached {:.2} Gb; ",
                mem_used as f32 / 1e9,
                mem_avail as f32 / 1e9,
                mem_cached as f32 / 1e9
            );
        } else {
            print!(
                "MEM: used {:.2} Gb, free {:.2} Gb, cached {:.2} Mb; ",
                mem_used as f32 / 1e9,
                mem_avail as f32 / 1e9,
                mem_cached as f32 / 1e6
            );
        }
    } else {
        print!(
            "MEM: used {:.2} Kb, free {:.2} Kb, cached {:.2} Kb; ",
            mem_used, mem_avail, mem_cached
        );
    }
