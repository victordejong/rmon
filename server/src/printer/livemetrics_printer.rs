...
use live_metrics_protobuf::live_metrics_message::{LiveCpuMessage,LiveMemMessage,LiveDiskMessage};
...
pub fn print_received_live_metrics(request: &Request<LiveMetricsMessage>) {
    ...
    let mem_message: &LiveMemMessage = match &request_message.mem {
        None => {
            println!("ERROR: Malformed MemMessage received from {} ({})!", request_message.hostname, remote_addr);
            return;
        }
        Some(mem) => mem
    };
    ...
    print!("Metrics update received for {} ({}):", request_message.hostname, remote_addr);
    print!("CPU: freq avg {:.2} GHz, load {} {} {}; ", cpu_message.freq / 1024.,
            &cpu_message.cpu_load.as_ref().unwrap().load_1m, &cpu_message.cpu_load.as_ref().unwrap().load_5m,
            &cpu_message.cpu_load.as_ref().unwrap().load_15m);

    print!("MEM: free {:.2} Gb, available {:.2} Gb, cached {:.2} Mb; ",
            mem_message.free as f32 / 1e9, mem_message.available as f32 / 1e9, mem_message.cached as f32 / 1e6);
